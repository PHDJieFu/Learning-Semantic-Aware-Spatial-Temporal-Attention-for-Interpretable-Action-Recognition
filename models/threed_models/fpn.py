        ...
        for i, dim in enumerate(inplanes):
            ...
            if ds_num < 1:
                ...
                for dsi in range(ds_num):
                    in_factor = 2 ** dsi
                    out_factor = 2 ** (dsi + 1)
                    op.append(ConvModule(dim * in_factor, dim * out_factor, kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1), bias=False))
...
class FPN(nn.Module):
    def __init__(self,
        ...
        spatial_modulation_config = Config(spatial_modulation_config) if isinstance(spatial_modulation_config, dict) else spatial_modulation_config
        temporal_modulation_config = Config(temporal_modulation_config) if isinstance(temporal_modulation_config, dict) else temporal_modulation_config
        upsampling_config = Config(upsampling_config) if isinstance(upsampling_config, dict) else upsampling_config
        downsampling_config = Config(downsampling_config) if isinstance(downsampling_config, dict) else downsampling_config
        # aux_head_config = Config(aux_head_config) if isinstance(aux_head_config, dict) else aux_head_config
        level_fusion_config = Config(level_fusion_config) if isinstance(level_fusion_config, dict) else level_fusion_config
        ...
        for i in range(0, self.num_ins, 1):
            ...
            if i < self.num_ins - 1:
                ...
                if downsampling_config is not None:
                    ...
                    downsampling_config.param.inplanes = planes             # 1024
                    downsampling_config.param.planes = planes               # 1024
                    downsampling_config.param.downsample_scale = downsampling_config.scales     # (2, 1, 1)
                    downsampling = Downampling(**downsampling_config.param)
                    self.downsampling_ops.append(downsampling)

        out_dims = level_fusion_config.out_channels             # 2048
    ...
    def forward(self, inputs):
        ...
        # if self.aux_head is not None:
            ...
            # x_fpn_supervised = self.aux_head(inputs[-2])
            # loss_fpn_supervised = 0.5 * F.cross_entropy(x_fpn_supervised, target)
