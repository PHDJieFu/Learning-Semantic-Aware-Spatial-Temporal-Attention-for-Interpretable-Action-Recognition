...
parser.add_argument('-s', '--shorter_side', default=[256], type=int, nargs="+", help='shorter side of the generated image, if two values are provided, '
...
parser.add_argument('-n', '--num_classes', type=int, help='number of classes', choices=[400, 600], default=400)
...
def create_train_video(shorter_side):
    ...
    with open(train_file_list, 'w') as f, open(train_fail_file_list, 'w') as f_w, concurrent.futures.ProcessPoolExecutor(max_workers=64) as executor:
        futures = [executor.submit(video_to_images, video, train_video_folder, train_img_folder, shorter_side) for video in train_videos]
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if frame_num == 0:
                ...
                print("{},{},{},{},train".format(label_id, youtube_id, time_start, time_end), file=f_w, flush=True)
            elif frame_num == -2:
                youtube_id = video_id[:11]
                time_start = int(video_id[12:18])
                time_end = int(video_id[19:25])
                print("{},{},{},{},train,missed".format(label_id, youtube_id, time_start, time_end), file=f_w, flush=True)
            else:
                print("{};1;{};{}".format(os.path.join('images/train', id_to_label[label_id], video_id), frame_num, label_id), file=f, flush=True)
            if curr_idx % 1000 == 0:
...
def create_val_video(shorter_side):
    print("Resizing to shorter side: {}".format(shorter_side))
    with open(val_file_list, 'w') as f, open(val_fail_file_list, 'w') as f_w, concurrent.futures.ProcessPoolExecutor(max_workers=36) as executor:
        futures = [executor.submit(video_to_images, video, val_video_folder, val_img_folder, shorter_side) for video in val_videos]
        total_videos = len(futures)
        curr_idx = 0
        print("label,youtube_id,time_start,time_end,split", file=f_w, flush=True)
        for future in concurrent.futures.as_completed(futures):
            ...
            if frame_num == 0:
                ...
                time_start = int(video_id[12:18])
                time_end = int(video_id[19:25])
                print("{},{},{},{},val".format(label_id, youtube_id, time_start, time_end), file=f_w, flush=True)
                # print("{},{},{},{},val".format(label_id, youtube_id, time_start, time_end), flush=True)
            elif frame_num == -2:
                youtube_id = video_id[:11]
                time_start = int(video_id[12:18])
                time_end = int(video_id[19:25])
                print("{},{},{},{},val,missed".format(label_id, youtube_id, time_start, time_end), file=f_w, flush=True)
            else:
                print("{};1;{};{}".format(os.path.join('images/val', id_to_label[label_id], video_id), frame_num, label_id), file=f, flush=True)
            if curr_idx % 1000 == 0:
...
def create_test_video(shorter_side):
    with open(test_file_list, 'w') as f, open(test_fail_file_list, 'w') as f_w, concurrent.futures.ProcessPoolExecutor(
            max_workers=36) as executor:
        futures = [executor.submit(video_to_images, video, test_video_folder, test_img_folder, shorter_side) for video in test_videos]
        total_videos = len(futures)
        curr_idx = 0
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if frame_num == 0:
                ...
                time_start = int(video_id[12:18])
                time_end = int(video_id[19:25])
                print("{},{},{},test".format(youtube_id, time_start, time_end), file=f_w, flush=True)
            else:
                print("{} 1 {}".format(os.path.join('images/test', video_id), frame_num), file=f, flush=True)
            print("{}/{}".format(curr_idx, total_videos))
            curr_idx += 1
    print("Completed")
