...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.gpu = gpu
    # 174        'train.txt'      'val.txt'      'test.txt'      ' '                '{:05d}.jpg'    3           ???
    ...
    if args.mean is not None:
        if args.modality == 'rgb':
            if len(args.mean) != 3:
                raise ValueError("When training with rgb, dim of mean must be 3.")
        elif args.modality == 'flow':
            if len(args.mean) != 1:
                raise ValueError("When training with flow, dim of mean must be 1.")
        mean = args.mean

    if args.std is not None:
        if args.modality == 'rgb':
            if len(args.std) != 3:
                raise ValueError("When training with rgb, dim of std must be 3.")
        elif args.modality == 'flow':
            if len(args.std) != 1:
                raise ValueError("When training with flow, dim of std must be 1.")
        std = args.std
    ...
    if args.distributed:
        ...
        # load pretained_model and fix module parameters except the fpn module and classifier layer
        pretrained_path = './snapshots/st2stv2-rgb-i3d-resnet-50-f16-multisteps-bs64-e76/model_best.pth.tar'
        pretrained_model = torch.load(pretrained_path)
        model_state_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_model.items() if k in model_state_dict}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.rank == 0:
            ...
            is_best = val_top1 > torch.tensor(best_top1).cuda()
            # is_best = val_top1 > best_top1
            best_top1 = max(val_top1, torch.tensor(best_top1).cuda())
