...
def arg_parser():
    ...
    # model definition
    parser.add_argument('--backbone_net', default='i3d_resnet', type=str, help='backbone network',
                        choices=list(MODEL_TABLE.keys()))
    parser.add_argument('-d', '--depth', default=50, type=int, metavar='N', help='depth of resnet (default: 18)',
                        choices=[18, 34, 50, 101, 152])
    parser.add_argument('--dropout', default=0.5, type=float, help='dropout ratio before the final layer')
    ...
    parser.add_argument('--pooling_method', default='max', choices=['avg', 'max'],
                        help='method for temporal pooling method')
    ...
    parser.add_argument('--blending_method', default='sum', choices=['sum', 'max'],
                        help='method for blending channels in TAM')
    ...
    parser.add_argument('-b', '--batch-size', default=96, type=int, metavar='N', help='mini-batch size (default: 256)')
    parser.add_argument('--lr', '--learning-rate', default=0.01, type=float, metavar='LR', help='initial learning rate')
    parser.add_argument('--lr_scheduler', default='multisteps', type=str, help='learning rate scheduler',
                        choices=['step', 'multisteps', 'cosine', 'plateau'])
    parser.add_argument('--lr_steps', default=[15, 30, 45, 55, 65], type=float, nargs="+", metavar='LRSteps',
                        help='[step]: use a single value: the periodto decay '
                             'learning rate by 10. '
    ...
    parser.add_argument('--epochs', default=83, type=int, metavar='N', help='number of total epochs to run')
    parser.add_argument('--resume', default='', type=str, metavar='PATH', help='path to latest checkpoint (default: none)')
     #  default='/home/daxiedong/code/action-recognition-pytorch/snapshots/st2stv2-rgb-i3d-resnet-50-f16-multisteps-bs96-e79/checkpoint.pth.tar',
    parser.add_argument('--pretrained', dest='pretrained', type=str, metavar='PATH', help='use pre-trained model')
    #  default='/home/daxiedong/code/action-recognition-pytorch/snapshots/st2stv2-rgb-i3d-resnet-50-f16-multisteps-bs64-e76/model_best.pth.tar',
    ...
    parser.add_argument('--datadir', default='/home/daxiedong/dataset/SSV2/Something-Somethingv2', metavar='DIR',
                        help='path to dataset file list')                            # /home/jygao/SSV2/
    parser.add_argument('--dataset', default='st2stv2', choices=list(DATASET_CONFIG.keys()), help='which dataset.')
    parser.add_argument('--threed_data', default=True, action='store_true', help='format data to 5D for 3D onv.')
    parser.add_argument('--input_size', default=112, type=int, metavar='N', help='spatial size')
    parser.add_argument('--disable_scaleup', action='store_true', help='do not scale up and then crop a small region, '
    ...
    parser.add_argument('--scale_range', default=[256, 320], type=int, nargs="+", metavar='scale_range',
    ...
    parser.add_argument('--print-freq', default=600, type=int, help='frequency to print the log during the training')
    parser.add_argument('--show_model', action='store_true', help='show model and then exit intermediately')

    # for testing and validation
    parser.add_argument('-e', '--evaluate', dest='evaluate', help='evaluate model on validation set')                # action='store_true'
    parser.add_argument('--num_crops', default=1, type=int, choices=[1, 3, 5, 10],
                        help='[Test.py only] number of crops.')
    parser.add_argument('--num_clips', default=1, type=int, help='[Test.py only] number of clips.')
