...
def get_augmentor(is_train, image_size, mean=None, std=None, disable_scaleup=False, is_flow=False, threed_data=False, version='v1', scale_range=None):
...
def build_dataflow(dataset, is_train, batch_size, workers=36, is_distributed=False):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=workers, pin_memory=True, sampler=sampler)
...
def train(data_loader, model, criterion, optimizer, epoch, display=100,
    ...
    with tqdm(total=len(data_loader)) as t_bar:
        for i, (images, target) in enumerate(data_loader):
            ...
            att_output, per_output, att_t, att_s = model(images, target)
            target = target.cuda(gpu_id, non_blocking=True)
            att_loss = criterion(att_output, target)
            # fpn_loss = F.cross_entropy(x_fpn_supervised, target)
            per_loss = criterion(per_output, target)
            # loss = att_loss + per_loss + 0.5 * fpn_loss
            loss = att_loss + per_loss

            # measure accuracy and record loss
            prec1, prec5 = accuracy(per_output, target)
...
def validate(data_loader, model, criterion, gpu_id=None):
    ...
    with torch.no_grad(), tqdm(total=len(data_loader)) as t_bar:
        ...
        for i, (images, target) in enumerate(data_loader):
            ...
            att_loss = criterion(att_output, target)
            # fpn_loss = F.cross_entropy(x_fpn_supervised, target)
            # att_layer_2_loss = criterion(att_layer_2_output, target)
            per_loss = criterion(per_output, target)
            # loss = att_loss + per_loss + 0.5 * fpn_loss
            loss = att_loss + per_loss
